...
public:
    ...
    struct KD_TREE_NODE
    {
        ...
        // For multi-thread tree rebuild
        std::mutex push_down_mutex;
        // Range of subtree
        float node_range_x[2], node_range_y[2], node_range_z[2];
        float alpha_bal = 0.5, alpha_del = 0.0;
    };
...
private:
    ...
    std::thread *rebuild_thread_ptr = nullptr;
    std::mutex termination_flag_mutex, rebuild_ptr_mutex, working_flag_mutex, search_flag_mutex;
    std::mutex rebuild_logger_mutex, points_deleted_rebuild_mutex;
    ...
    void Search(KD_TREE_NODE *root, int k_nearest, PointType point, MANUAL_HEAP &q, float max_dist);
