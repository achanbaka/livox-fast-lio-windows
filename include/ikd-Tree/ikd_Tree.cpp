...
{
    rebuild_thread_ptr = new std::thread([this]() { multi_thread_rebuild(); });
...
{
    termination_flag_mutex.lock();
    termination_flag = true;
    termination_flag_mutex.unlock();
    if (rebuild_thread_ptr && rebuild_thread_ptr->joinable())
    {
        rebuild_thread_ptr->join();
        delete rebuild_thread_ptr;
        rebuild_thread_ptr = nullptr;
    }
}
...
{
    static thread_local MANUAL_HEAP q(16);
    q.reserve(2 * k_nearest);
    q.clear();
    ...
    Nearest_Points.resize(static_cast<size_t>(k_found));
    Point_Distance.resize(static_cast<size_t>(k_found));
    for (int i = k_found - 1; i >= 0; --i)
    {
        Nearest_Points[static_cast<size_t>(i)] = q.top().point;
        Point_Distance[static_cast<size_t>(i)] = q.top().dist;
        q.pop();
    }
    return;
}
...
void KD_TREE<PointType>::Search(KD_TREE_NODE *root, int k_nearest, PointType point, MANUAL_HEAP &q, float max_dist)
{
    ...
    {
        retval = root->push_down_mutex.try_lock() ? 0 : -1;
        if (retval == 0)
        {
            Push_Down(root);
            root->push_down_mutex.unlock();
        }
        else
        {
            root->push_down_mutex.lock();
            root->push_down_mutex.unlock();
        }
    }
