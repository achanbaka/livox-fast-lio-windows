...
- **回放播放控制** — Foxglove 播放条可同步当前时间，支持暂停、继续和倍速播放（seek 随机跳转暂未支持）
...
Foxglove 的播放时间会跟随 SLAM 处理时间广播。LVX 和 Bag 回放支持在 Foxglove 中暂停、继续和调整播放速度；拖动进度条进行 seek 随机跳转目前会暂停回放并在日志中提示未支持。原因是 FAST-LIO 状态和地图由历史 LiDAR/IMU 积分得到，不能只跳文件指针，否则滤波状态和地图会不一致。
...
暂不支持拖动进度条 seek 到任意时间点。收到 seek 请求时程序会暂停并输出提示，因为 FAST-LIO 的滤波状态、局部地图和完整累计地图都依赖历史帧顺序积分；要支持真正 seek，需要重置 SLAM 并从目标时间前重新回放一段数据。
